...
class AnalisadorNotasEscolares:
    def __init__(self, fonte_dados, nome_pasta="gráficos"):
        self.dados = []
        self.dados_gerais = {}
        ...
        self.categorias_faltas = ['0 faltas', '1 falta', '2 faltas', '3 faltas']
        self.cores = ['green', 'yellow', 'orange', 'red']
    ...
    def processar_dados(self):
        def porcentagem_presenca(lista_faltas):
            presenca = 0
            quantidade = 0
            for x in range(4):
                presenca += (4 - x) * lista_faltas[x]
                quantidade += lista_faltas[x]
            return float(f"{100 * presenca / (quantidade * 4):.2f}")

        self.dados_gerais = {}
        colunas = ['redacao 1', 'ciencias humanas 1', 'ciencias exatas 1',
                   'redacao 2', 'ciencias humanas 2', 'ciencias exatas 2']
        colunas_secundarias = ["soma", "soma_periodo_1", "soma_periodo_2"]
        self.dados_gerais["turmas"] = ["3TA", "3TB", "3TIA", "3TIB", "4TA", "4TB", "4TI"]
        self.dados_gerais["cursos"] = ["TA", "TB", "TI"]
        self.dados_gerais['faltas'] = [0, 0, 0, 0]
        self.dados_gerais['faltas_ano'] = [[0, 0, 0, 0], [0, 0, 0, 0]]
        self.dados_gerais['quantidade_ano'] = [0, 0]
        self.dados_gerais['faltas_turma'] = {turma: [0, 0, 0, 0] for turma in self.dados_gerais["turmas"]}
        self.dados_gerais['quantidade_turma'] = {turma: 0 for turma in self.dados_gerais["turmas"]}
        self.dados_gerais['faltas_curso'] = {curso: [0, 0, 0, 0] for curso in self.dados_gerais["cursos"]}
        self.dados_gerais['quantidade_curso'] = {curso: 0 for curso in self.dados_gerais["cursos"]}
        self.dados_gerais['faltas_simulados'] = {coluna: 0 for coluna in colunas}
        self.dados_gerais['quantidade_faltou_todas'] = 0
        for coluna in colunas + colunas_secundarias:
            self.dados_gerais[coluna] = {"quantidade": 0, "soma": 0, "min": 10000, "max": 0}
        para_excluir = []
        for i in range(len(self.dados)):
            # pegar o curso, considera que a turma é [ano][curso][divisão sala]
            self.dados[i]["curso"] = self.dados[i]["turma"][1:3]
            faltas = 0
            for coluna in colunas:
                # Processar faltas
                if self.dados[i][coluna] == 0:
                    faltas += 1
                else:
                    self.dados_gerais[coluna]["soma"] += int(self.dados[i][coluna])
                    self.dados_gerais[coluna]["quantidade"] += 1

                    if self.dados[i][coluna] < self.dados_gerais[coluna]["min"]:
                        self.dados_gerais[coluna]["min"] = self.dados[i][coluna]
                    if self.dados[i][coluna] > self.dados_gerais[coluna]["max"]:
                        self.dados_gerais[coluna]["max"] = self.dados[i][coluna]
            if faltas > 3:
                self.dados_gerais['quantidade_faltou_todas'] += 1
                para_excluir.append(i)
                continue
            self.dados[i]["faltas"] = faltas
            self.dados_gerais["faltas"][faltas] += 1
            self.dados_gerais["faltas_ano"][self.dados[i]["serie"] - 3][faltas] += 1
            self.dados_gerais["faltas_turma"][self.dados[i]["turma"]][faltas] += 1
            self.dados_gerais["faltas_curso"][self.dados[i]["curso"]][faltas] += 1
            self.dados_gerais["quantidade_ano"][self.dados[i]["serie"] - 3] += 1
            self.dados_gerais["quantidade_turma"][self.dados[i]["turma"]] += 1
            self.dados_gerais["quantidade_curso"][self.dados[i]["curso"]] += 1
            # Calcular as novas somas
            for j in range(2):
                d = str(j + 1)
                periodo = 'soma_periodo_' + d
                self.dados[i][periodo] = (self.dados[i]['redacao ' + d] +
                                          self.dados[i]['ciencias humanas ' + d] +
                                          self.dados[i]['ciencias exatas ' + d])

                if self.dados[i][periodo] > 0:
                    self.dados_gerais[periodo]["soma"] += self.dados[i][periodo]
                    self.dados_gerais[periodo]["quantidade"] += 1

                    if self.dados[i][periodo] < self.dados_gerais[periodo]["min"]:
                        self.dados_gerais[periodo]["min"] = self.dados[i][periodo]
                    if self.dados[i][periodo] > self.dados_gerais[periodo]["max"]:
                        self.dados_gerais[periodo]["max"] = self.dados[i][periodo]

            # Atualizar soma dos dados gerais
            self.dados_gerais["soma"]["soma"] += int(self.dados[i]["soma"])
            self.dados_gerais["soma"]["quantidade"] += 1

            if self.dados[i]["soma"] < self.dados_gerais["soma"]["min"]:
                self.dados_gerais["soma"]["min"] = self.dados[i]["soma"]
            if self.dados[i]["soma"] > self.dados_gerais["soma"]["max"]:
                self.dados_gerais["soma"]["max"] = self.dados[i]["soma"]

        para_excluir.reverse()
        for i in para_excluir:
            del self.dados[i]

        for coluna in colunas + colunas_secundarias:
            try:
                self.dados_gerais[coluna]["media"] = (self.dados_gerais[coluna]["soma"] /
                                                      self.dados_gerais[coluna]["quantidade"])
            except ZeroDivisionError:
                self.dados_gerais[coluna]["media"] = 0

        print("\nAnálise de faltas:")
        for i in range(4):
            print(f" Alunos com {i} faltas: {self.dados_gerais['faltas'][i]}")
        print(f" Alunos que faltaram todos os simulados: {self.dados_gerais['quantidade_faltou_todas']}")
        print(f"\nPercentual de presença total: {porcentagem_presenca(self.dados_gerais['faltas'])}%")
        for i in range(2):
            print(f"Percentual de presença {i + 3}° ano: {porcentagem_presenca(self.dados_gerais['faltas_ano'][i])}%")
        for curso, faltas in self.dados_gerais["faltas_curso"].items():
            print(f"Percentual de presença {curso}: {porcentagem_presenca(faltas)}%")
        for turma, faltas in self.dados_gerais["faltas_turma"].items():
            print(f"Percentual de presença {turma}: {porcentagem_presenca(faltas)}%")
    ...
    def plotar_distribuicao_faltas(self, mostrar):
        def plot():
            ...
            bars.append(axes[0, 0].bar(self.categorias_faltas, self.dados_gerais["faltas"], color=self.cores))
            axes[0, 0].bar_label(bars[0])
            axes[0, 0].set_title('Alunos por Número de Faltas')
            axes[0, 0].set_ylim(0, sum(self.dados_gerais["faltas"]))

            # Gráfico 2: Faltas por ano
            faltas_ano = [[self.dados_gerais["faltas_ano"][0][x],
                           self.dados_gerais["faltas_ano"][1][x]] for x in range(4)]

            brs = [np.arange(len(faltas_ano[1]))]
            for i in range(len(faltas_ano) - 1):
                brs.append([x + grossuraBarra for x in brs[i]])
            ...
            axes[0, 1].set_xticks([r + grossuraBarra for r in range(len(faltas_ano[1]))],
                                  ["3°", "4°"])
            axes[0, 1].set_ylim(0, sum(self.dados_gerais["faltas"]))
            axes[0, 1].set_title('Faltas por ano')
            axes[0, 1].tick_params(axis='x')

            # Gráfico 2: Faltas por curso
            faltas_curso = [[self.dados_gerais["faltas_curso"]["TA"][x],
                             self.dados_gerais["faltas_curso"]["TB"][x],
                             self.dados_gerais["faltas_curso"]["TI"][x]] for x in range(4)]
            ...
            axes[1, 0].set_xticks(distancias_cursos, self.dados_gerais["cursos"])
            axes[1, 0].set_ylim(0, sum(self.dados_gerais["faltas"]))
